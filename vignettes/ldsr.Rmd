...
bm <- PCR_reconstruction(P1annual, P1pc, start.year = 1600)
```
...
```{r, eval=FALSE}
set.seed(100)
lds <- LDS_reconstruction(P1annual, u, v, start.year = 1600, num.restarts = 100)
```
...
```{r, eval=FALSE}
cv <- cvLDS(P1annual, u, v, start.year = 1600, Z = Z, num.restarts = 100)
```
...
bmcv <- cvPCR(P1annual, pc, start.year = 1600, Z = Z)
dt1 <- bmcv$metrics.dist
...
plot_reconstruction <- function(s, target, period, benchmark = NULL) {
    ...
    q <- ggplot(s) +
      ...
      labs(x = 'Year', y = 'Catchment state X [-]')
    
    plot_grid(p, q, ncol = 1, align = 'v', rel_heights = c(1.5, 1))
}
```
