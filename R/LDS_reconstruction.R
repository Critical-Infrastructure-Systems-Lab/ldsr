learnLDS_restart <- function(y, u, v, mu, init = NULL, num.restart, niter = 1000, tol = 1e-5, return.init = FALSE, parallel = FALSE) {
    ...
    if (parallel) {
        models <- foreach(init.val = init) %dopar% learnLDS(y, u, v, mu, init.val, niter, tol)
    } else {
        models <- lapply(init, function(init.val) learnLDS(y, u, v, mu, init.val, niter, tol))
    }
