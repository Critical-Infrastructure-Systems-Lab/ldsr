...
#' @export
learnLDS_restart0 <- function(y, u, v, init,
    ...
    if (parallel) {
        ...
        models <- foreach(init.val = init) %dopar% learnLDS0(y, u, v, init.val, niter, tol)
        stopCluster(cl)
    } else {
        models <- lapply(init, function(init.val) learnLDS0(y, u, v, init.val, niter, tol))
    }
...
#' @export
LDS_reconstruction0 <- function(Qa, u, v, init = NULL, num.restart = 100,
                               niter = 1000, tol = 1e-5, return.init = TRUE,
                               parallel = FALSE) {
    ...
    results <- learnLDS_restart0(y, u, v, init, niter, tol, return.init, parallel)
...
#' @export
cvLDS0 <- function(Qa, u, v, init = NULL, num.restart = 20,
    ...
    if (parallel)  {
        ...
        cv.results <- foreach(omit = Z) %dopar% {
            y2 <- y
            y2[omit] <- NA
            learnLDS_restart0(y2, u, v, init, niter, tol, return.init = FALSE)
        }
        stopCluster(cl)
    } else {
